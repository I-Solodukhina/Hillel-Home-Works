...
def get_address():
	try:
		...
		for row in addrFile:
			if len(row) == 10:
				_, _, _, _, street_address, landmark, area, city, state, pincode = row
				if street_address:
					all_addrs['street_address'].append(street_address)
				if landmark:
					all_addrs['landmark'].append(landmark)
				if area:
					all_addrs['area'].append(area)
				if city:
					all_addrs['city'].append(city)
				if state:
					all_addrs['state'].append(state)
				if pincode:
					all_addrs['pincode'].append(pincode)
		full_addr = {key: choice(value) for key, value in all_addrs.items() if value}
		return full_addr
