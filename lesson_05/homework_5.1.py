...
search_criteria_price = search_criteria[2]

filtered_cars = {}

for key, value in car_data.items():
    color, year, engine, car_type, price = value
    if year >= search_criteria_year and engine >= search_criteria_engine_volume and price <= search_criteria_price:
        filtered_cars[key] = price

sorted_cars = sorted(filtered_cars.items(), key=lambda item: item[1])[:5]
for key, price in sorted_cars:
    print(key, car_data[key])
